...
class Agent():
    ...
    def step_0_rifle_man(self, enemys, game_map):
        ...
        for y in range(g_height):
            if (player == 0):
                for x in range(g_width // 2 - 3, g_width // 2 + 3):
                    if game_map[y][x].tile_type != 0:
                        tiles_type_2.append(game_map[y][x])
            else:
                for x in range(g_width // 2 - 3, g_width // 2 + 3):
        ...
        if not tiles_type_2:
            return middle_x // 2, middle_y / 2
        tiles_type_2 = [ tile for tile in tiles_type_2
        if (tile.x, tile.y) not in position_list
        ]
        tiles_type_2.sort(key=lambda tiles: (abs(tiles.x - middle_x), abs(tiles.y - middle_y), tiles.x), reverse = True)
        best_tile = tiles_type_2[0]
        if (player == 0):
            return best_tile.x - 1, best_tile.y
        else:
    ...
    def performe_move(self, enemys, my_agents, game_map):
        ...
        if enemy != None and self.cooldown == 0:
            ...
            print(f"{self.agent_id}; MOVE {self.x} {self.y}; THROW {enemy.x} {enemy.y}")
        else:
