...
class Agent():
    ...
    def find_best_cover(self, enemys, game_map):
        ...
        if (self.x - 1 >= 0):
            if (game_map[self.y][self.x - 1].protection_score > prot_score):
                x = self.x - 1
                y = self.y
                prot_score = game_map[self.y][self.x - 1].protection_score
        if (self.x + 1 < g_width):
            if (game_map[self.y][self.x + 1].protection_score > prot_score):
                x = self.x + 1
                y = self.y
                prot_score = game_map[self.y][self.x + 1].protection_score
        if (self.y - 1 >= 0):
            if (game_map[self.y - 1][self.x].protection_score > prot_score):
                x = self.x
                y = self.y - 1
                prot_score = game_map[self.y - 1][self.x].protection_score
        if (self.y + 1 < g_width):
            if (game_map[self.y + 1][self.x].protection_score > prot_score):
                x = self.x
                y = self.y + 1
    ...
    def find_best_enemy(self, enemys, game_map):
        ...
        if enemys:
            ...
            for enemy in enemys:
                if (abs(self.x - enemy.x) + abs(self.y - enemy.y) <= 4):
    ...
    def quick_throw_i_am_lazy(self, enemys, game_map):
        throw_to_performe = (0, 0)
        for throw in throws:
            distance = manhatan_distance(self.x, self.y, throw[0], throw[1])
            if (distance < 4 and distance > 2):
                throw_to_performe = throw
                print(f"{self.agent_id}; MOVE {x} {y}; THROW {throw_to_performe[0]} {throw_to_performe[1]}")
                throws.remove(throw)
                return
            elif (distance < 2):
                throws.remove(throw)
                ...
                throw_to_performe = throw
        print(f"{self.agent_id}; MOVE {throw_to_performe[0]} {throw_to_performe[1]}")
    ...
    def perform_move(self, enemys, game_map):
        ...
        self.quick_throw_i_am_lazy(enemys, game_map)
        # if enemy != None:
